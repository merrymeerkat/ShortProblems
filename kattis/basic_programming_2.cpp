...
int one(int N, vector<int> numbers); // Sorted
...
int frequent(int N, vector <int> numbers){
    ...
    //using count sourt (arguably not the best here... array is unecessarily long.
    // Since the sequence is sorted, we could also try just using a simple counter)
    int counting[2000000] = { 0 };

    int siz = numbers.size();

    //add countings to respective buckets
    for (int i = 0; i < siz; ++i){
        counting[numbers.at(i)]++;
    }

    //check if any bucket has >N/2
    for (int i = 0; i < siz; ++i){
        if (counting[numbers.at(i)] > N/2){
            cout << numbers.at(i) << "\n";
            return 0;
        }
    }
