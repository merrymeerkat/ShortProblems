#include <iostream>
#include <vector>
#include <deque>
#include <map>

using namespace std;

typedef pair<int,int> ii;
map <ii, int> parent;
map <ii, int> dist;
string grid[1010];
const int inf = 1e9;

int R, C;
int dr[8] = {-1, -1, 0, 1, 1, 1, 0, -1};
int dc[8] = {0, 1, 1, 1, 0, -1, -1, -1};

int main(){

    ios_base::sync_with_stdio(false); cin.tie(NULL); cout.tie(NULL);

    cin >> R >> C;
    for (int r = 0; r < R; r++)
        cin >> grid[r];

    int Q; cin >> Q;
    int sr, sc, tr, tc, nd;
    deque <pair<ii, int> > deq;
    int cost[R][C];
    while (Q--){
        cin >> sr >> sc >> tr >> tc;
        ii source = ii(sr - 1, sc - 1);
        for (int r = 0; r < R; r++){
            for (int c = 0; c < C; c++)
                cost[r][c] = inf;
        }

        cost[sr - 1][sc -1] = 0;
        deq.push_back({source, 0});
        int r, c, nr, nc, num;
        bool exp;
        while (!deq.empty()){
            auto [u, d] = deq.front(); deq.pop_front();
            r = u.first; c = u.second;
            for (int i = 0; i < 8; i++){
                nr = r + dr[i]; nc = c + dc[i];
                num = grid[r][c] - '0';
                exp = (num != i); 
                nd = d + (exp);
                if (nr < 0 || nc < 0 || nr >= R || nc >= C || cost[nr][nc] <= nd) continue;
                cost[nr][nc] = nd;
                if (nr == tr - 1 && nc == tc - 1) break;
                if (exp) // if we are moving against the current
                    deq.push_back({{nr, nc}, nd});
                else // if we are moving along the current
                    deq.push_front({{nr,nc}, nd});
            }
        }

        cout << cost[tr - 1][tc - 1] << '\n';
    }
}
