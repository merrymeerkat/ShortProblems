...
ll get_dist(pair<int, int> a, pair<int,int> b){
    ll dist = pow((ll)a.first - b.first, 2) + pow((ll)a.second - b.second, 2);
    return dist;
}
...
int main(){
    ...
    // Dijkstra
    set <pair<int, pair<int, int> > > dist_and_nodes; 
...
//test    for (auto [d, n]: dist_and_nodes){
